var HiddenRolls = HiddenRolls || {
    ...
    showHelp: function(who){
        ...
        helpMsg += "  -q, --quiet:      obcure roll formulas\n";
    ...
    handleHideMessage: function(tokens, msg){
        ...
        var replaceInlines = _.noop;

        if (_.contains(tokens, "-h") || _.contains(tokens, "--help")){
            return HiddenRolls.showHelp(msg.who);
        }

        if (_.contains(tokens,"-v") || _.contains(tokens,"--verbose")){
            verbose = true;
            cmd = cmd.replace(/(\s|^)(?:-v|--verbose)\b\s+/g, '$1');
        }
        if (_.contains(tokens,"-q") || _.contains(tokens,"--quiet")){
            quiet = true;
            cmd = cmd.replace(/(\s|^)(?:-q|--quiet)\b\s+/g, '$1');
        }
        ...
        switch(tokens[0]){
            ...
            case "!hiderolls":
                ...
                if (verbose || quiet){
                    ...
                    sendChat(msg.who+(quiet?' (quiet)':''), "/w gm " + cmd.replace(/\$\[\[\d+\]\]/g, replaceInlines));
                }
                break;
            case "!hidetotal":
            case "!hidetotals":
                ...
                sendChat(msg.who+(quiet?' (quiet)':''), "/w gm " + hiddenCmd);
                if ((verbose) && (!playerIsGM(msg.playerid)) && !quiet){
                    // verbose mode: whisper totals to original sender too
                    sendChat(msg.who, '/w "' + msg.who + '" ' + hiddenCmd);
                }
                break;
            case "!hideall":
                ...
                else if (!playerIsGM(msg.playerid) && !quiet){
                    // normal mode: whisper expression to sender
                    sendChat(msg.who, '/w "' + msg.who + '" ' + cmdExpr);
                }
                break;
            default:
                return HiddenRolls.showHelp(msg.who);
